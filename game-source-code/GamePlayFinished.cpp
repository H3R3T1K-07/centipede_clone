...
namespace Centipede
{
	GamePlayFinished::GamePlayFinished(GameDataRef data, int score) : _data(data), _score(score)
	{

	}

	void GamePlayFinished::Init()
	{
        ...
        _gameCompleteTitle.setFont(this->_data->resource.GetFont("Splash Screen Font"));

        _gameCompleteTitle.setCharacterSize(70);

        _gameCompleteTitle.setString("YOU WIN");

        
        _gameCompleteTitle.setPosition(sf::Vector2f((_data->window.getSize().x / 2) - 180 , (_data->window.getSize().y / 2) - 200));

        _gameCompleteTitle.setColor(sf::Color::Blue);
	...
	void GamePlayFinished::HandleInput()
	{
		sf::Event event;
	...
	void GamePlayFinished::Update(float dt)
	{

	}

	void GamePlayFinished::Draw(float dt)
	{
		this->_data->window.clear();

		this->_data->window.draw(this->_background);
        
        this->_data->window.draw( this->_gameCompleteTitle );
        
        _data->window.draw(_scoreText);
        _data->window.draw(_highScoreText);
         _data->window.draw(_scoreTitle);
        _data->window.draw(_highScoreTitle);
        _data->window.draw(_splashInstruction);
